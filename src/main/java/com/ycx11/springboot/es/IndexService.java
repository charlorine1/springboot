package com.ycx11.springboot.es;

import com.ycx11.springboot.util.HtmlTool;
...
/*    @Test
    public void createIndex() throws Exception {
        IndicesExistsResponse resp = client.admin().indices().prepareExists("searchdemo").execute().actionGet();
        if(resp.isExists()){
            client.admin().indices().prepareDelete("searchdemo").execute().actionGet();
        }
        client.admin().indices().prepareCreate("searchdemo").execute().actionGet();
...
//				.field("search_analyzer", "ik_max_word").endObject()
        ...
        PutMappingRequest mapping = Requests.putMappingRequest("searchdemo").type("htmlbean").source(builder);
        ...
        if(file.isDirectory()){
            ...
            try {
                ...
                if(bean!=null){
                    ...
                    //写索引
                    client.prepareIndex("searchdemo", "htmlbean").setSource(dataMap).execute().actionGet();
                }
            } catch (Throwable e) {
                e.printStackTrace();
            }
...
//		q1.to(40);
        ...
        if(wr.getIndex()==1){
            resp = client.prepareSearch("searchdemo")
                    .setTypes("htmlbean")
                    .setQuery(q)
